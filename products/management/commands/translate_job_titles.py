...
class Command(BaseCommand):
    ...
    required_columns = ['English Label', 'Auto Dutch Translation', 'Auto German Translation',
                        'Better Dutch translation', 'Better German translation']
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                'Success. {} German translations, {} Dutch translations applied.'.format(total_de_translations_applied,
                                                                                         total_nl_translations_applied)))
    ...
    def __translate(row: dict, force_translate) -> Tuple[int, int]:
        ...
        for job_title in qs:  # type: JobTitle
            if job_title.name_de and not force_translate:
                continue
            job_title.name_de = row['Better German translation'] if row[
                'Better German translation'] else row['Auto German Translation']
            de_translations_applied += 1

            if job_title.name_nl and not force_translate:
                continue
            job_title.name_nl = row['Better Dutch translation'] if row['Better Dutch translation'] else \
                row['Auto Dutch Translation']
