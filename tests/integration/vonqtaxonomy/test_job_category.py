...
class JobCategoryTaxonomyTestCase(AuthenticatedTestCase):
    ...
    def test_returns_right_value(self):
        response = self.client.get(reverse("vonqtaxonomy:job-category") + "?job_function_name=Badabim")
    ...
    def test_returns_404_when_using_unknown_value(self):
        response = self.client.get(reverse("vonqtaxonomy:job-category") + "?job_function_name=foobar")
